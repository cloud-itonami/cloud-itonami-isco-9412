(ns kitchen.actor
  "KitchenSupportPracticeActor — the ISCO-08 9412 community kitchen actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one kitchen operation request
  ...
  The unconditional invariant: the KitchenSupportPracticeAdvisor can never
  directly commit a record the KitchenSupportPracticeGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [kitchen.advisor :as advisor]
            [kitchen.governor :as governor]
            [kitchen.store :as store]))

(defn build-graph
  "Build a compiled KitchenSupportPracticeActor graph. `store` implements
  `kitchen.store/Store`. `advisor` implements
  `kitchen.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :kitchen-id (:kitchen-id proposal)
                                    :payload proposal}]
